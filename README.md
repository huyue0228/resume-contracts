...
业务平台和简历分析引擎之间的公开契约，版本 5.0.0。

v5 新增 `budget.max_context_tokens`，与累计 `max_tokens` 分开；新增 `safe_trace.budget`、`safe_trace.rounds` 及工具的安全错误代码、字段位置和重复标记。严格校验的两端必须同步升级，不能将 v5 诊断发给旧 v4 消费端。评估结果和独立分配协议保持不变。

停止原因分别为 `token_limit`、`next_request`、`context_limit`、`turn_limit`、`tool_limit`、`no_progress` 等受控枚举；`usage_source` 区分模型报告、估算和混合计量。诊断禁止包含原文、提示词、工具参数、原始服务响应或密钥。
...
协议为 `resume-analysis/v5`，HTTP 入口为 `POST /v2/tasks/execute`。请求只携带已准入范围，不携带历史志愿、学校规则或 HC；结果不包含业务动作。
...
先安装 `build setuptools>=68 wheel`，再执行 `make check package RELEASE_VERSION=v5.0.0`。
仓库脚本生成 wheel/sdist、SHA256SUMS，并在仓库外的临时环境离线安装 wheel 验证。
产物在 `dist/v5.0.0/`，拒绝覆盖已有目录。构建依赖由运行环境预置，脚本不下载它们。
...
v5 任务 `candidate.application_assessment` 每次只携带一个当前投递评估标准（`scope.jobs` 长度恰好为 1），结果 `resume-application-assessment/v1` 最多含一个契合度结果。`scope.tag_catalog` 是受控标签字典，`profile.tags` 含 code、confidence、status 和必填原文 evidence；标签所属范围和原文语义校验由 Kernel 与平台共同执行。保留 `ResumeTextV2` 正文格式和既有评分权重，入池、复核和分配不进入模型输出。v4 删除 `scope.taxonomy`，旧版本请求不再受支持。


5.0.0 保留独立分配协议 `resume-allocation/v1`（任务 `pool.candidate_allocation`），结果 `resume-allocation-plan/v1`。筛选协议升级为 v5，不保留专业大类词表。模拟服务也提供 `/v2/allocation/capabilities` 和 `/v2/allocation/tasks/execute`。
