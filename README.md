...
先安装 `build setuptools>=68 wheel`，再执行 `make check package RELEASE_VERSION=v3.1.0`。
仓库脚本生成 wheel/sdist、SHA256SUMS，并在仓库外的临时环境离线安装 wheel 验证。
产物在 `dist/v3.1.0/`，拒绝覆盖已有目录。构建依赖由运行环境预置，脚本不下载它们。
...
3.1.0 新增独立分配协议 `resume-allocation/v1`（任务 `pool.candidate_allocation`），结果 `resume-allocation-plan/v1`。现有筛选 v3 保持兼容。模拟服务也提供 `/v2/allocation/capabilities` 和 `/v2/allocation/tasks/execute`。
